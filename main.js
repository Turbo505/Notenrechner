...
function addResult(i, wert){
    document.getElementById(fächer[i]).value = wert;
}
...
function newGK(){
    let gkNumber = fächer[fächer.length-1].charAt(2) + 1
    let gk = "gk" + gkNumber;
    fächer.push(gk);
    alert(`Array${fächer}Neu${gk}`)
    genGK(gk);
}
...
function genGK(gk){
    ...
    document.getElementById("outputCalculated").innerHTML = `${document.getElementById("outputCalculated").innerHTML}<br> <input id=gk${gkAnzahl}readonly>`
}
